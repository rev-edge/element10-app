...
begin
 ...
 if tg_table_name='e10_unique_item_grade_assessments'then
  ...
  if new.action='assert'then
   ...
   elsif new.commercial_event_id is not null and new.disposition_kind='return'then
    select e.occurred_at into v_source_time from public.e10_current_inventory_lifecycle_events e where e.organization_id=new.organization_id and e.id=new.commercial_event_id and e.event_type='return'and e.unique_item_id=new.unique_item_id;
   end if;
...
begin
 ...
 if p_kind='grade_assessment'then
  ...
  if auth.uid()is distinct from actor or not e10.has_org_cap(p_org,'act.curate_market_analytics')then raise exception using errcode='42501',message='inventory_evidence_review_denied';end if;
