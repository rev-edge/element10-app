...
declare result jsonb; new_batch uuid; current_predecessor uuid; predecessor_status text; current_status text;
begin
 ...
 select status,supersedes_intake_batch_id into current_status,current_predecessor from public.e10_intake_batches
 ...
 if current_predecessor is null and current_status='committed' then raise exception using errcode='55000',message='committed_ordinary_batch_cannot_be_retrofitted'; end if;
 if current_predecessor is null then update public.e10_intake_batches
   set supersedes_intake_batch_id=p_supersedes_batch_id,review_revision=review_revision+1,updated_at=now()
