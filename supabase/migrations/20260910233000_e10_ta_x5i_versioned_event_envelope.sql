...
begin
  ...
  if p_occurred_at_precision not in ('exact','date','unknown') or ((p_occurred_at_precision='unknown')<>(p_occurred_at is null)) then raise exception using errcode='22023',message='event_occurrence_precision_invalid'; end if;
