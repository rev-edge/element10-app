...
returns table(condition_state text,grader_code text,grade_label text,grade_qualifier text,autograph_designation text,assessment_id uuid)
...
returns table(unique_item_id uuid,catalog_variant_id uuid,origin_event_id uuid,origin_at timestamptz,holding_evidence text)
language sql stable security definer set search_path=public as $$
 with origins as(
  select e.unique_item_id,e.id,e.occurred_at,row_number()over(partition by e.unique_item_id order by e.occurred_at desc,e.recorded_at desc,e.id desc)rn,
   count(*)over(partition by e.unique_item_id)open_count
  from public.e10_current_inventory_lifecycle_events e
  where e.organization_id=p_org and e.event_type in('acquisition','receipt')and e.occurred_at<=p_cutoff
   and not exists(select 1 from public.e10_current_inventory_disposition_links d where d.organization_id=p_org and d.unique_item_id=e.unique_item_id and d.origin_event_id=e.id and d.disposed_at<=p_cutoff)
 ),known as(
  select o.unique_item_id,o.id origin_event_id,o.occurred_at origin_at,case when o.open_count=1 then'episode'else'episode_ambiguous'end evidence
  from origins o where o.rn=1
 ),record_only as(
  select u.id,null::uuid,null::timestamptz,'record_only'
  from public.e10_unique_items u where u.organization_id=p_org and u.created_at<=p_cutoff
   ...
   and not exists(select 1 from public.e10_current_inventory_disposition_links d where d.organization_id=p_org and d.unique_item_id=u.id and d.disposed_at<=p_cutoff)
 )
 select h.unique_item_id,u.catalog_variant_id,h.origin_event_id,h.origin_at,h.evidence from(select*from known union all select*from record_only)h
...
returns table(evidence_id uuid,amount numeric,observed_at timestamptz,recorded_at timestamptz,source_kind text,source_reference text,selected_basis text)
language sql stable security definer set search_path=public as $$
 with grade as(select*from e10.inventory_grade_at(p_org,p_item,p_cutoff)),eligible as(
  ...
  where v.organization_id=p_org and(v.unique_item_id=p_item or v.unique_item_id is null and v.catalog_variant_id=p_variant)
   ...
   and(v.condition_state='all_conditions'or v.condition_state='raw'and g.condition_state='raw'
 ...
 select e.id,e.amount,e.observed_at,e.recorded_at,e.source_kind,e.source_reference,e.basis from eligible e
...
returns table(snapshot_id uuid,population_count bigint,observed_at timestamptz,source_kind text,source_reference text)
language sql stable security definer set search_path=public as $$
 ...
 where p.organization_id=p_org and p.catalog_variant_id=p_variant and p.observed_at<=p_cutoff
  and(p.condition_state='all_conditions'or p.condition_state='raw'and g.condition_state='raw'
...
begin
 ...
 with close_h as materialized(select*from e10.inventory_holdings_at(p_org,p_closing_cutoff)),open_h as materialized(select*from e10.inventory_holdings_at(p_org,p_opening_cutoff)where p_opening_cutoff is not null),all_ids as materialized(select unique_item_id from close_h union select unique_item_id from open_h),facts as materialized(
  select i.unique_item_id,coalesce(c.catalog_variant_id,o.catalog_variant_id)catalog_variant_id,c.holding_evidence closing_holding,o.holding_evidence opening_holding,
   ...
   oe.amount opening_value,oe.evidence_id opening_evidence_id,
   pop.population_count,pop.observed_at population_observed_at,
   case when can_cost then cost.amount end actual_cost,
  ...
  left join lateral e10.inventory_estimate_at(p_org,i.unique_item_id,coalesce(c.catalog_variant_id,o.catalog_variant_id),p_closing_cutoff,make_interval(days=>p_freshness_days),btrim(p_method),btrim(p_method_version),p_currency)ce on c.unique_item_id is not null
  left join lateral e10.inventory_estimate_at(p_org,i.unique_item_id,coalesce(c.catalog_variant_id,o.catalog_variant_id),p_opening_cutoff,make_interval(days=>p_freshness_days),btrim(p_method),btrim(p_method_version),p_currency)oe on o.unique_item_id is not null and p_opening_cutoff is not null
  left join lateral e10.inventory_population_at(p_org,coalesce(c.catalog_variant_id,o.catalog_variant_id),i.unique_item_id,p_closing_cutoff)pop on true
  left join lateral(select m.amount from public.e10_current_market_observations m where m.organization_id=p_org and m.unique_item_id=i.unique_item_id and m.observation_kind='acquisition_cost'and m.currency=p_currency and m.occurred_at<=p_closing_cutoff order by m.occurred_at desc,m.recorded_at desc,m.id desc limit 1)cost on can_cost
   ...
   'comparable_count',count(*)filter(where opening_holding is not null and closing_holding is not null and opening_value is not null and closing_value is not null),'market_movement',sum(closing_value-opening_value)filter(where opening_holding is not null and closing_holding is not null and opening_value is not null and closing_value is not null),
   'acquisition_count',count(*)filter(where opening_holding is null and closing_holding is not null),'acquisition_endpoint_value',sum(closing_value)filter(where opening_holding is null and closing_holding is not null),
   'disposal_count',count(*)filter(where opening_holding is not null and closing_holding is null),'disposal_endpoint_value',sum(opening_value)filter(where opening_holding is not null and closing_holding is null),
   'uncomparable_count',count(*)filter(where(opening_holding is null or closing_holding is null or opening_value is null or closing_value is null)),
   'unknown_cost_count',count(*)filter(where closing_holding is not null and(can_cost and actual_cost is null)),'actual_cost_access',case when can_cost then'authorized'else'not_authorized'end)j from facts),
 ...
 select(select j from totals),(select count(*)from page)>p_limit,coalesce(jsonb_agg(jsonb_build_object('unique_item_id',unique_item_id,'holding_evidence',closing_holding,'value',closing_value,'valuation_evidence_id',closing_evidence_id,'evidence_observed_at',closing_evidence_at,'source_kind',closing_source,'selected_basis',closing_basis,'population_count',population_count,'population_observed_at',population_observed_at,'actual_cost',actual_cost,'actual_cost_access',actual_cost_access)order by unique_item_id),'[]'),(select unique_item_id from shown order by unique_item_id desc limit 1)
