...
language sql stable security definer set search_path=public as $$
 with primary_origins as(
  ...
  where e.organization_id=p_org and e.occurred_at<=p_cutoff and(
   e.event_type='acquisition'or e.event_type='receipt'and not exists(
  ...
  select e.unique_item_id,e.id,e.occurred_at,e.recorded_at,
  ...
  from primary_origins e where not exists(select 1 from public.e10_current_inventory_dispositions d
   where d.organization_id=p_org and d.unique_item_id=e.unique_item_id and d.origin_event_id=e.id and d.disposed_at<=p_cutoff)
 ),known as(
  select o.unique_item_id,o.id origin_event_id,o.occurred_at origin_at,o.recorded_at origin_recorded_at,case when o.open_count=1 then'episode'else'episode_ambiguous'end evidence
...
begin
 ...
 with close_h as materialized(select*from e10.inventory_holdings_at(p_org,p_closing_cutoff)),open_h as materialized(select*from e10.inventory_holdings_at(p_org,p_opening_cutoff)where p_opening_cutoff is not null),all_ids as materialized(select unique_item_id from close_h union select unique_item_id from open_h),facts as materialized(
  ...
  left join lateral(select m.amount from public.e10_current_market_observations m where m.organization_id=p_org and m.unique_item_id=i.unique_item_id and m.observation_kind='acquisition_cost'and m.currency=p_currency and m.occurred_at>=c.origin_at and m.occurred_at<=p_closing_cutoff order by m.occurred_at desc,m.recorded_at desc,m.id desc limit 1)cost on can_cost and c.holding_evidence='episode'
