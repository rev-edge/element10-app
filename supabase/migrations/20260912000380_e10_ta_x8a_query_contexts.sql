...
revoke all on public.e10_query_contexts,public.e10_query_context_commands from public,anon,authenticated;
grant all on public.e10_query_contexts,public.e10_query_context_commands to service_role;
...
begin raise exception using errcode='55000',message='query_context_command_immutable';end $$;
...
begin
 ...
 if found then if old.command_type<>'create'or old.request_fingerprint<>fp then raise exception using errcode='22023',message='query_context_idempotency_mismatch';end if;if auth.uid()is distinct from a or not e10.is_org_member(p_org)then raise exception using errcode='42501',message='query_context_create_denied';end if;return old.result||jsonb_build_object('replay',true);end if;
...
begin
 ...
 perform pg_advisory_xact_lock(hashtextextended(p_org::text||'|'||a::text||'|x8-query-context|'||btrim(p_idempotency_key),0));
 select command_type,request_fingerprint,result into old from public.e10_query_context_commands where organization_id=p_org and actor_id=a and idempotency_key=btrim(p_idempotency_key);
 if found then if old.command_type<>'revoke'or old.request_fingerprint<>fp then raise exception using errcode='22023',message='query_context_idempotency_mismatch';end if;if auth.uid()is distinct from a or not e10.is_org_member(p_org)then raise exception using errcode='42501',message='query_context_revoke_denied';end if;return old.result||jsonb_build_object('replay',true);end if;
 ...
 if not found or(c.actor_id is distinct from a and not e10.is_org_admin(p_org))or c.revoked_at is not null or auth.uid()is distinct from a or not e10.is_org_member(p_org)then raise exception using errcode='42501',message='query_context_revoke_denied';end if;
...
revoke all on function e10.reject_query_context_command_change(),e10.x8_query_context_actor(uuid,uuid,boolean)from public,anon,authenticated;
grant execute on function e10.reject_query_context_command_change(),e10.x8_query_context_actor(uuid,uuid,boolean)to service_role;
