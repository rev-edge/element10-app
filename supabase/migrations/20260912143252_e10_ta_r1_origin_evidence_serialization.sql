-- R1 follow-up: serialize receipt-origin creation, make conflicts explicit,
-- and distinguish newly written origins from historical compatibility repair.

create or replace function e10.ensure_receipt_origin_evidence(p_org uuid,p_receipt_id uuid) returns void
language plpgsql security definer set search_path=public as $$
declare r record;captured_retroactively boolean;
begin
  ...
  captured_retroactively:=current_setting('e10.new_receipt_origin',true) is distinct from 'on';
  for r in
  ...
  loop
    ...
    values(p_org,'receipt',1,'inventory_item',r.inventory_item_id,
      coalesce(r.received_at,now()),'exact',p_org::text||':receipt-origin:'||r.line_id,
      'native','receipt-ledger','stock_receipt_line:'||r.line_id,r.line_id::text,
      p_receipt_id::text,'native_system',jsonb_build_object(
        'movement_id',r.inventory_movement_id,'movement_type','intake',
        'on_hand_delta',r.accepted_quantity,'reserved_delta',0,
        'source_action','receive','reason_code','intake','receipt_id',p_receipt_id,
        'receipt_line_id',r.line_id,'lot_id',r.lot_id,
        'received_quantity',r.received_quantity,'accepted_quantity',r.accepted_quantity,
        'damaged_quantity',r.damaged_quantity,'quarantined_quantity',r.quarantined_quantity,
        'captured_retroactively',captured_retroactively),auth.uid(),
    ...
    if not exists(select 1 from public.e10_commercial_events ce
      ...
      raise exception using errcode='23505',message='receipt_evidence_conflict';
    end if;
  end loop;
end $$;
...
begin
  ...
  perform set_config('e10.new_receipt_origin','off',true);
  if auth.uid() is null or not exists(select 1 from public.e10_organizations where id=p_org and status='active')
    or not e10.is_org_member(p_org) or not e10.has_org_cap(p_org,'act.create_receiving')
...
begin
  select destination_location_id,disposition_revision into destination,before_revision from public.e10_stock_receipts
