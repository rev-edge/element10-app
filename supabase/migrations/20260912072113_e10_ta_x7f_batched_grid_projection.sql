-- TA-X7f staging performance corrective, revision 3. Keep the canonical X7b
-- contribution projection unchanged. These private helpers batch only the fields
-- consumed by the X7f grid and its diagnostics.

create function e10.customer_spend_grid_contributions(
...
create function e10.customer_spend_grid_diagnostics(
  p_org uuid,p_from timestamptz,p_to timestamptz,p_observation_cutoff timestamptz,
  p_currency text,p_customer uuid default null,p_purchase_kind text default null,
  p_location uuid default null,p_channel text default null,p_product uuid default null,
  p_configuration uuid default null,p_copy uuid default null,p_session uuid default null,
  ...
  with attributed_transactions as materialized(
    select t.id,t.occurred_at,t.occurred_at_precision,ea.effective_customer_id
    from public.e10_customer_transactions t
    cross join lateral e10.customer_transaction_effective_attribution(p_org,t.id)ea
    where t.organization_id=p_org and t.currency=p_currency
  ),authorized_lines as materialized(
    select t.* from attributed_transactions t
    join public.e10_customer_transaction_lines l
      on l.organization_id=p_org and l.transaction_id=t.id
    where e10.can_view_customer_financials_at(p_org,l.location_id)
    ...
    'unknown_occurrence_line_count',count(*)filter(where
      (occurred_at_precision='unknown'or occurred_at is null)
      and effective_customer_id is not null
      and(p_customer is null or effective_customer_id=p_customer)),
    'unattributed_authorized_line_count',case when p_customer is null then count(*)filter(where
      occurred_at_precision<>'unknown'and occurred_at is not null
      and occurred_at>=p_from and occurred_at<p_to and occurred_at<p_observation_cutoff
      and effective_customer_id is null)else null end,
...
begin
  ...
  if p_cursor is not null then
    ...
    from e10.customer_spend_grid_contributions(
  ...
  with raw as materialized(
    select * from e10.customer_spend_grid_contributions(p_org,v_from,p_to,p_observation_cutoff,p_currency,p_customer,p_purchase_kind,p_location,p_channel,p_product,p_configuration,p_copy,p_session,p_capture_source)
  ), grouped as(
  ...
  v_diagnostics:=e10.customer_spend_grid_diagnostics(p_org,v_from,p_to,p_observation_cutoff,p_currency,p_customer,p_purchase_kind,p_location,p_channel,p_product,p_configuration,p_copy,p_session,p_capture_source);
