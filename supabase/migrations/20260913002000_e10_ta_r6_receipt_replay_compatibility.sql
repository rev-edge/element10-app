-- Preserve semantic replay for pre-R6 single-line receipts regardless of the
-- textual scale originally used to fingerprint equal numeric quantities.
...
begin
 ...
 select sr.id receipt_id,sr.status,sr.received_at,rl.id receipt_line_id,rl.accepted_quantity,rl.damaged_quantity,rl.quarantined_quantity,rl.inventory_lot_id,rl.inventory_movement_id,l.inventory_item_id,l.lot_code,a.purchase_order_line_id
 into prior from public.e10_stock_receipts sr join public.e10_stock_receipt_lines rl on(rl.organization_id,rl.stock_receipt_id)=(sr.organization_id,sr.id)
 ...
 if found then
  ...
  select coalesce(jsonb_agg(jsonb_build_object('id',(x->>'id')::uuid,'quantity',(x->>'quantity')::numeric)order by(x->>'id')::uuid),'[]')into requested_allocations from jsonb_array_elements(coalesce(p_expected_allocations,'[]'))x;
  if row(prior.purchase_order_line_id,prior.inventory_item_id,prior.accepted_quantity,prior.damaged_quantity,prior.quarantined_quantity,prior.lot_code,stored_allocations)
    is distinct from row(p_purchase_order_line_id,p_inventory_item_id,coalesce(p_accepted_quantity,0),coalesce(p_damaged_quantity,0),coalesce(p_quarantined_quantity,0),p_lot_code,requested_allocations)
...
language plpgsql security definer set search_path=public as $$declare prior record;begin
 ...
 select * into prior from public.e10_lot_reservations where organization_id=p_org and idempotency_key=p_idempotency_key;
 if found then
  if row(prior.lot_id,prior.quantity,prior.source_type,prior.source_id)is distinct from row(p_lot_id,p_quantity,'break_session'::text,p_break_session_id::text)then raise exception using errcode='22023',message='idempotency_key_mismatch';end if;
 ...
 begin return public._e10_org_lot_reserve_r6(p_org,p_lot_id,trim_scale(p_quantity),p_break_session_id,p_idempotency_key);exception when unique_violation then raise exception using errcode='55000',message='lot_reservation_already_active';end;
end $$;
...
language plpgsql security definer set search_path=public as $$declare prior record;stored_label text;begin
 if auth.uid()is null or not e10.has_org_cap(p_org,'act.reserve_inventory')then raise exception using errcode='42501',message='reserve_inventory_denied';end if;
 ...
 if found then
  if row(prior.lot_id,prior.quantity,prior.source_type,prior.source_id,prior.show_label)is distinct from row(p_lot_id,p_quantity,p_demand_type,btrim(p_demand_reference),btrim(p_demand_label))then raise exception using errcode='22023',message='idempotency_key_mismatch';end if;
  return jsonb_build_object('ok',true,'replay',true,'reservation_id',prior.id,'legacy_reservation_id',prior.legacy_reservation_id,'movement_id',prior.movement_id,'status',prior.status,'consumed_quantity',prior.consumed_quantity);
 end if;
 begin return public._e10_org_lot_reserve_for_demand_r6(p_org,p_lot_id,trim_scale(p_quantity),p_demand_type,p_demand_reference,p_demand_label,p_idempotency_key);exception when unique_violation then raise exception using errcode='55000',message='lot_reservation_already_active';end;
end $$;

revoke all on function public.e10_org_lot_reserve(uuid,uuid,numeric,uuid,text),public.e10_org_lot_reserve_for_demand(uuid,uuid,numeric,text,text,text,text)from public,anon;
grant execute on function public.e10_org_lot_reserve(uuid,uuid,numeric,uuid,text),public.e10_org_lot_reserve_for_demand(uuid,uuid,numeric,text,text,text,text)to authenticated,service_role;
