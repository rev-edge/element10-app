-- TA-X7f approved customer spend grid v2.
--
-- Runtime contract: docs/TA_X7F_CUSTOMER_SPEND_GRID_PLAN.md at f0d7069.
-- This migration is intentionally additive. The v1 spend functions remain callable
-- and the X8 dispatcher must preserve its v1 path for callers with no v2-only key.
--
-- Implementation invariants:
--   * all line and aggregate filters run before keyset pagination;
--   * known-history is recorded posted history, never claimed complete history;
--   * known-history attendance is explicitly unavailable;
--   * 100001 eligible authorized contributions fail without a partial response;
--   * customer display-name changes invalidate cursors through the reporting revision;
--   * public/anon receive no execution privilege.

-- A grid cursor can bind a display-name ordering only when every mutation of
-- the projected customer identity invalidates the reporting revision. The
-- generic bump function derives the tenant from NEW or OLD as appropriate.
create trigger e10_reporting_revision_customer_grid_identity
after insert or delete or update of display_name,status on public.e10_customers
for each row execute function e10.bump_reporting_dataset_revision();

comment on trigger e10_reporting_revision_customer_grid_identity on public.e10_customers
is 'Invalidates customer-grid cursors when projected customer identity or visibility changes.';

create function e10.customer_attendance_denominator_if_bounded(
  p_mode text,p_org uuid,p_customer uuid,p_from timestamptz,p_to timestamptz,
...
begin
  if p_mode<>'bounded'then return;end if;
  return query select d.attended_sessions,d.coverage_complete
  from e10.customer_attendance_denominator(
    p_org,p_customer,p_from,p_to,p_observation_cutoff,p_timezone,p_week_start,p_session)d;
end $$;
revoke all on function e10.customer_attendance_denominator_if_bounded(text,uuid,uuid,timestamptz,timestamptz,timestamptz,text,integer,uuid)from public,anon,authenticated;
grant execute on function e10.customer_attendance_denominator_if_bounded(text,uuid,uuid,timestamptz,timestamptz,timestamptz,text,integer,uuid)to service_role;

create function public.e10_org_customer_spend_grid_v2(
...
begin
  ...
  if p_window_mode='known_history' then
    select count(*) into v_count from(select 1 from e10.official_customer_spend_contributions(p_org,v_from,p_to,p_observation_cutoff,p_currency,p_customer,p_purchase_kind,p_location,p_channel,p_product,p_configuration,p_copy,p_session,p_capture_source) limit 100001)s;
...
-- Preserve the reviewed X8a dispatcher byte-for-byte as the v1 delegate. The
-- same public signature below selects v2 only when window_mode is present.
alter function public.e10_org_typed_query(uuid,uuid,text,jsonb)
rename to e10_org_typed_query_v1;
revoke all on function public.e10_org_typed_query_v1(uuid,uuid,text,jsonb)
from public,anon,authenticated;
grant execute on function public.e10_org_typed_query_v1(uuid,uuid,text,jsonb) to service_role;

create function public.e10_org_typed_query(
  p_org uuid,p_context_id uuid,p_operation text,p_args jsonb
) returns jsonb language plpgsql volatile security definer set search_path=public as $$
declare actor uuid;res jsonb;safe_items jsonb;
begin
  if p_operation<>'customer.spend_summary' or not(p_args?'window_mode') then
  ...
  res:=public.e10_org_customer_spend_grid_v2(
    p_org,p_args->>'window_mode',(p_args->>'from')::timestamptz,
    (p_args->>'to')::timestamptz,(p_args->>'observation_cutoff')::timestamptz,
    p_args->>'currency',p_args->>'timezone',(p_args->>'week_start')::integer,
    (p_args->>'customer')::uuid,p_args->>'purchase_kind',(p_args->>'location')::uuid,
    p_args->>'channel',(p_args->>'product')::uuid,(p_args->>'configuration')::uuid,
  ...
  res:=jsonb_set(res,'{items}',safe_items,true);
  if res->'next_cursor' is not null and res->'next_cursor'<>'null'::jsonb then
    res:=jsonb_set(res,'{next_cursor}',(res->'next_cursor')-'display_name'-'normalized_display_name',true);
  end if;
  if e10.x8_query_context_actor(p_org,p_context_id,false)is distinct from actor then
    raise exception using errcode='42501',message='x8_query_denied';end if;
  return e10.x8_query_envelope(p_org,p_context_id,p_operation,p_args,res,
    'effective_customer',jsonb_build_object('money',p_args->>'currency'),
    '["source_history_completeness"]'::jsonb);
end $$;
