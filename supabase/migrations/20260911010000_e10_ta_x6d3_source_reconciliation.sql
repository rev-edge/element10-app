...
create table public.e10_customer_transaction_reconciliation_cases (
  ...
  foreign key(organization_id,claimed_posted_line_id) references public.e10_customer_transaction_lines(organization_id,id),
...
create table public.e10_customer_transaction_evidence_links (
  ...
  unique(organization_id,id),unique(organization_id,decision_id),
  foreign key(organization_id,case_id) references public.e10_customer_transaction_reconciliation_cases(organization_id,id),
...
create table public.e10_customer_transaction_source_claims (
  id uuid primary key default gen_random_uuid(),organization_id uuid not null,source_kind text not null check(source_kind in ('manual','import','native')),
  source_connection_id text,source_line_id text not null,posted_line_id uuid,reconciliation_case_id uuid,created_at timestamptz not null default now(),
...
begin
  ...
  return new;
exception when unique_violation then raise exception using errcode='23505',message='customer_transaction_source_already_claimed';
end $$;
revoke all on function e10.claim_customer_transaction_line_source() from public,anon,authenticated;
grant execute on function e10.claim_customer_transaction_line_source() to service_role;
create trigger e10_customer_transaction_line_source_claim_trg after insert on public.e10_customer_transaction_lines for each row execute function e10.claim_customer_transaction_line_source();
...
alter table public.e10_customer_transaction_source_claims enable row level security;
revoke all on public.e10_customer_transaction_reconciliation_cases,public.e10_customer_transaction_reconciliation_decisions,public.e10_customer_transaction_evidence_links,public.e10_customer_transaction_source_claims from public,anon,authenticated;
grant all on public.e10_customer_transaction_reconciliation_cases,public.e10_customer_transaction_reconciliation_decisions,public.e10_customer_transaction_evidence_links,public.e10_customer_transaction_source_claims to service_role;
...
declare v_fp text;v_old record;v_id uuid:=gen_random_uuid();v_result jsonb;v_claim record;
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended(p_org::text||'|posted|source|'||p_source_kind||'|'||coalesce(p_source_connection_id,'')||'|'||p_source_component_id,0));
  select posted_line_id,reconciliation_case_id into v_claim from public.e10_customer_transaction_source_claims where organization_id=p_org and source_kind=p_source_kind and coalesce(source_connection_id,'')=coalesce(p_source_connection_id,'') and source_line_id=p_source_component_id;
  if found and v_claim.reconciliation_case_id is not null then raise exception using errcode='23505',message='reconciliation_source_component_already_opened';end if;
  insert into public.e10_customer_transaction_reconciliation_cases(id,organization_id,source_kind,source_connection_id,source_event_id,source_component_id,currency,observed_merchandise_amount,observed_shipping_amount,observed_tax_amount,claimed_posted_line_id,evidence,idempotency_key,request_fingerprint,created_by)
  values(v_id,p_org,p_source_kind,nullif(btrim(p_source_connection_id),''),btrim(p_source_event_id),btrim(p_source_component_id),p_currency,p_merchandise_amount,p_shipping_amount,p_tax_amount,v_claim.posted_line_id,p_evidence,p_idempotency_key,v_fp,auth.uid());
  if v_claim.posted_line_id is null then insert into public.e10_customer_transaction_source_claims(organization_id,source_kind,source_connection_id,source_line_id,reconciliation_case_id) values(p_org,p_source_kind,nullif(btrim(p_source_connection_id),''),btrim(p_source_component_id),v_id);end if;
...
begin
  ...
  if p_action='link' then
    ...
    if p_match_basis='durable_external_id' and (v_case.claimed_posted_line_id is distinct from p_transaction_line_id or v_line.capture_source is distinct from v_case.source_kind or coalesce(v_line.source_connection_id,'') is distinct from coalesce(v_case.source_connection_id,'') or v_line.source_line_id is distinct from v_case.source_component_id) then raise exception using errcode='22023',message='durable_source_identity_does_not_match_line';end if;
  end if;
