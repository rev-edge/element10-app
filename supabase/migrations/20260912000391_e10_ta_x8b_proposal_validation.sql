...
begin
 ...
 if p_operation='purchase_order.create'then
  ...
  if jsonb_typeof(p_values->'lines')is distinct from'array'or jsonb_array_length(p_values->'lines')not between 1 and 200 then missing:=array_append(missing,'lines');
   ...
   if jsonb_typeof(line->'line_no')is distinct from'number'or jsonb_typeof(line->'ordered_quantity')is distinct from'number'or(line->>'line_no')::numeric<>trunc((line->>'line_no')::numeric)or(line->>'line_no')::numeric<=0 or line?'estimated_unit_cost'and jsonb_typeof(line->'estimated_unit_cost')not in('number','null')then raise exception using errcode='22023',message='action_proposal_line_type_invalid';end if;
   q:=(line->>'ordered_quantity')::numeric;if q<=0 or q::text in('NaN','Infinity','-Infinity')then missing:=array_append(missing,format('lines[%s].ordered_quantity',n));end if;if line->>'estimated_unit_cost'is not null then q:=(line->>'estimated_unit_cost')::numeric;if q<0 or q::text in('NaN','Infinity','-Infinity')then raise exception using errcode='22023',message='action_proposal_line_cost_invalid';end if;end if;
