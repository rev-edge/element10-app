...
| Mixed evidence | implemented | `tests/ta_x7d0_market_evidence_test.sql`, typed ask/sale/estimate evidence; `tests/ta_x7d2_screener_cohorts_test.sql`, completed-sale contribution only |
...
| Auction truth | implemented | `tests/ta_x5i_versioned_event_envelope_test.sql`, typed event/observation kinds; `tests/ta_x7e0_inventory_evidence_test.sql`, ask/estimate/sale separation. No disappearance-to-sale transition exists. |
...
| Scope switch | implemented | `tests/ta_x8a_query_context_test.sql` and `ta_x8a_context_concurrent_test.js`, context/org mismatch and post-wait revocation; `ta_x8b_permission_matrix_test.sql`, foreign draft denial |
...
| Hold vs sale | implemented | `tests/ta_x6g_native_break_sale_test.js`, hold has no spend, commit writes one provisional sale, exact retry adds none |
| Refund/resale | implemented | `tests/ta_x6d_customer_transaction_adjustments_test.js`, full refund/cancellation/reinstatement; `tests/ta_x6g_native_break_sale_test.js`, release/resale sequence identity |
...
| Mixed/unclassified import | implemented | `tests/ta_x6c_customer_transaction_posting_test.sql`, retail/break/unclassified line preservation and no channel guessing |
...
| Observation kind separation | implemented | `tests/ta_x5i_versioned_event_envelope_test.sql` and `ta_x7d0_market_evidence_test.sql`, strict kinds and no inventory/customer posting side effect |
...
| Grading granularity | implemented | `tests/ta_x7e0_inventory_evidence_test.sql`, grader/grade/qualifier applicability and unknown-bucket preservation |
...
| PUR-02 actual cost suggestion | implemented | `ta_x3f_supplier_workspace_test.sql` exact supplier/configuration/currency/as-of history; `ta_x4e_receipt_batch_test.sql` excludes unreceived estimates and preserves explicit received cost |
...
| PUR-05 physical acceptance | implemented | `ta_x4e_receipt_batch_test.sql` accepted/damaged/quarantined/shortage/partial quantities; X4f reversal and X4g disposition suites |
...
| Value evidence | `ta_x7e0_inventory_evidence_test.sql`, distinct cost/ask/sale/estimate kinds |
...
| Ordinary writer delegation | `ta_x8b_commit_delegation_test.sql`, supported PO/customer operations only |
...
| Scope switch and retention | `ta_x8b_permission_matrix_test.sql`, foreign context denied while original draft remains |
| External dispatch disabled | `ta_x8c_outbox_claim_ack_test.sql`, zero seeded consumer and no dispatch function/job |
