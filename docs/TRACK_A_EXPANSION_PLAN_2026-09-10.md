...
| TA-X5 | Partial, through X5g CI/staging verified | Typed intake and resolver lifecycle; reviewed evidence commit; native receipt/movement event links; immutable manual correction and reviewed re-import supersession lineage; atomic corrected durable-source and explicit source-less multirow imports; eligible-current interpretation; product/configuration/unique-item observations; kind/identity correction; stable idempotency and hostile-tenant proofs | **Required before TA-X5 is complete:** sequential and concurrent cycle/reverse-link acceptance coverage; card-variant-level catalog-only observations without ownership; complete the remaining lifecycle event families required by the framework |
