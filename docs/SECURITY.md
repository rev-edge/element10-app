...
- **Handle-verify race fix (A6a.1):** `e10_verify_handle_claim` is now race-free — the final `UPDATE` is conditional on `status='pending' AND expires_at > now()` (0 rows → `claim_not_pending_or_expired`), so a concurrent rejection or an expiry can never be overwritten regardless of interleaving, in addition to the advisory-lock + verified-only-unique backstops. Covered by `tests/a6a1_verify_bundle_test.sql` (reject-then-verify, expired-then-verify, two-pending-race) in CI.
