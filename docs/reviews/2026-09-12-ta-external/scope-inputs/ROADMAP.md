...
> Workflow update (2026-09-07): [Continuous authorized work](CONTINUOUS_WORK.md) supersedes routine plan-review, literal stop and sprint redispatch instructions. Historical status is not current progress; read canonical BOARD.md. Product dependencies, verification and genuine user decisions remain binding.
...
## September 10 expanded-vision queue

The owner authorized the [expansion framework](EXPANSION_FRAMEWORK.md) and evidence-backed reconciliation. This additive queue preserves historical gates below; it does not mark them accepted or authorize production cutover.

1. Reconcile environment status and requirement coverage; retain source identities.
2. PO trust revisions in separate tested candidates: internal/vendor comments, then permission-bound destination and historical actual-cost suggestion. Do not source "last paid" from unreceived PO estimates.
3. Extend PF-C5 preflight to PO-first and invoice/direct-purchase-first receiving, document matching and credits. Preserve PF-M4 location/approval and PF-M1 quantity/cost rules.
4. Capture typed commercial lifecycle and exact identity evidence before implementing authoritative intake/listing flows. Analytics UI can wait; missing historical events cannot be recreated honestly later.
5. Prove shared core with finished-goods and individual-item non-card fixtures; retain optional Cards and avoid a general ERP build.
6. Prototype configurable catalog screening and wishlist with monitoring off. External market data, integrations and AI remain gated on access, rights, bounded queries and acceptance evidence.

See [audit and coverage](EXPANSION_AUDIT_2026-09-10.md). No new literal stop phrases or automatic checkpoints are added.
...
**Owner priority, September 10:** this functional workflow is critical. Preserve its full product-to-live task loop while closing lifecycle/persistence gaps; do not reduce it to a visual refresh or bypass inventory/preparation/security dependencies.
...
- **6.4 Low-friction governed identity:** stable subject ID, aliases and source IDs; sport/league and time-bounded team affiliations are relationships, not a rigid player-under-current-team hierarchy. Support multi-subject cards. Keep intake light with progressive enrichment and a visible unresolved queue.
- **6.5 AI-assisted hygiene:** suggest source-backed matches, normalization and duplicate candidates with confidence and explanation. Ambiguous merges/splits and canonical changes require authorized review; preserve lineage and recoverable corrections. Never merge by name alone or treat model confidence as verification. See framework section 11.
...
**Owner priority, September 10:** sorting, filtering and slicing are core product behavior. One shared query/grid contract across cards, products, players, customers and reporting: composable typed filters, grouping, useful columns, stable sort with explicit unknowns, saved views, source drill-through and permitted export. Filters/aggregates cover the full authorized dataset, not just the loaded page. Page/all-results selection scope must be explicit. Verify correctness and representative-volume responsiveness, keyboard access and return context.
...
- **9.4 Customer intelligence grid:** optional spend ranges, order frequency, first/last purchase, recency, average order value, channels, locations and observed product/category/player affinities. Support composable filtering/grouping and saved views, with customer-private fields permission-filtered.
- **9.5 Customer metrics contracts:** derive totals from deduplicated order-line/sale/refund facts; define currency, discounts, tax/shipping treatment, time window and data coverage. Separate merchandise spend, contribution, refunds, store-credit use and outstanding balances. Never count the same order twice through live-session records and imported actuals. See framework section 11 and customer acceptance cases.
- **9.6 Retail and break spend:** separate purchase kind from channel/capture source. Manual entry, supported marketplace imports (Whatnot first; Fanatics Live feasibility to verify), and native board sales capture evidence/provisional activity, not automatically posted spend. Offer optional authorized review-and-post from board activity. Official customer totals use eligible approved, posted transactions and adjustments; provisional totals stay separately labeled. Link customer/session/slot/purchase and reconcile later imports without double counting. Holds, bids and assignments are not spend. Posting and settlement stay distinct; viewer companion remains read-only for commercial actions.
- **9.7 Customer engagement visuals:** report observed session duration, distinct attendance per week and spend by break/product with explicit denominators, time windows and coverage. Companion presence is not video watch time. Add bounded, deduplicated presence evidence and privacy controls before claiming attendance metrics. Product allocations must conserve sale totals; mixed/unallocated values remain visible. Grids, charts and AI share framework section 11 metric definitions.
...
**Owner priority:** cross-catalog relational reporting is the primary analytical experience, not a succession of player/card detail pages. One query must combine subject, rookie designation, grader/grade, parallel and release dimensions across products, with configurable groups/measures and preserved drill-down context. Catalog, owned-copy and sales-observation scopes have distinct counts and measures. This refines the research-panel-first emphasis in the comparative review; governed identity and evidence remain prerequisites.
...
- **11.5a Valuation evidence and portfolio change:** implement the [comparative review refinements](CARD_DATA_COMPARATIVE_REVIEW_2026-09-10.md): population snapshots, grade-aware series, versioned estimates and portfolio acquisition/disposal versus repricing attribution. Complete identity/grading/provider mapping prerequisites first. No external historical-sales access assumed.
- **11.6 Internal-data-first catalog and valuation analytics:** filtering by governed parallel/color/finish and other facets, price history and collection-value views work with owned lifecycle data plus validated manual/spreadsheet observations, without live connectors. Add typed staging, provenance, identity review and deduplication before analytics acceptance. Distinguish cost, ask, sale and estimate; show coverage, unknown valuations and sample limits. External feeds enrich rather than gate these capabilities. See expansion framework section 6.

## Phase 12 — Final navigation expansion

Dependent on trustworthy modules.

## Phase 13: Collector showcase (deferred final product phase)

Owner-approved roadmap placement only. A presentation-first collection hub, separate from near-term shop execution: personal themed collections, uploaded raw-card/slab front and optional back images, editable surface details and personal notes, custom ordering, curated backgrounds/colors, gentle shimmer, enlarge/flip interactions and revocable read-only sharing. No pricing, ownership verification, catalog matching or tracking integration required for its initial version. Free-tier positioning is a possibility, not an approved pricing commitment.

Keep a future connection to the tracking layer through optional permission-checked catalog/owned-item references. Gallery membership is many-to-many and never duplicates inventory, transfers ownership or posts transactions. A standalone upload remains valid without those references. Linkage must not expose shop-private fields or implicitly publish personal notes. Public views use an explicit allowlist and preview, with private-by-default collections and tested revocation. See expansion framework section 12. No showcase implementation is authorized by this roadmap entry; current phases stay prioritized.
