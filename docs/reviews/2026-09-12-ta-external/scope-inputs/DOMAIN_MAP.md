...
## September 10 expansion boundary

[EXPANSION_FRAMEWORK.md](EXPANSION_FRAMEWORK.md) refines this map for generic individually tracked items, typed extensions, purchasing documents, lifecycle events, market observations, composable screening and wishlist/monitoring. Core plus Cards was already approved; no second inventory engine is introduced. Existing `Today` labels below are historical: organization/module isolation is present in staging, not production, and reviewed purchasing/singles screens are prototypes. See the [environment-qualified audit](EXPANSION_AUDIT_2026-09-10.md).
