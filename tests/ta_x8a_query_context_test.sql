...
begin;
do $$
declare o uuid:=gen_random_uuid();o2 uuid:=gen_random_uuid();u uuid:=gen_random_uuid();v uuid:=gen_random_uuid();r uuid:=gen_random_uuid();r2 uuid:=gen_random_uuid();j jsonb;j2 jsonb;cid uuid;
begin
 ...
 if has_function_privilege('anon','public.e10_org_create_query_context(uuid,text,integer,text)','execute')or has_function_privilege('anon','public.e10_org_revoke_query_context(uuid,uuid,text)','execute')or has_function_privilege('authenticated','e10.x8_query_context_actor(uuid,uuid,boolean)','execute')then raise exception'context function ACL leak';end if;
end $$;
rollback;
