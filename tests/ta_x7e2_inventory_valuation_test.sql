...
begin;
do $$
declare o uuid:=gen_random_uuid();user_id uuid:=gen_random_uuid();role_id uuid:=gen_random_uuid();release_id uuid:=gen_random_uuid();variant_id uuid:=gen_random_uuid();item1 uuid:=gen_random_uuid();item2 uuid:=gen_random_uuid();origin1 uuid:=gen_random_uuid();origin2 uuid:=gen_random_uuid();cost_obs uuid:=gen_random_uuid();j jsonb;j2 jsonb;cursor_value text;
begin
 ...
 if(j#>>'{summary,closing_holding_count}')::int<>2 or(j#>>'{summary,closing_value}')::numeric<>170 or(j#>>'{summary,market_movement}')::numeric<>20 or(j#>>'{summary,acquisition_count}')::int<>1 or(j#>>'{summary,acquisition_endpoint_value}')::numeric<>50 or j#>>'{summary,actual_cost_access}'<>'not_authorized'or j#>>'{items,0,actual_cost}'is not null then raise exception'X7e valuation summary %',j;end if;
 if(j#>>'{items,0,value}')::numeric<>120 or(j#>>'{items,0,population_count}')::int<>23 then raise exception'grade-aware closing selection %',j;end if;
 ...
 j:=public.e10_org_inventory_valuation_coverage(o,'local-index','1','USD','2026-01-07','2026-01-04',365,100,null);if j#>>'{summary,actual_cost_access}'<>'authorized'or(j#>>'{items,0,actual_cost}')::numeric<>80 then raise exception'actual cost permission boundary %',j;end if;
 reset role;
end $$;
rollback;
