...
x.run=randomUUID();x.session=randomUUID();x.items=['compete','cas','dispwin','revwin','auth','replay','corrdisp','corrrev','location','org','reserveauth','single','consumeauth','releaseauth','batchloc','legacyreserve','legacyrelease','legacyconsume','singleauth','originrace'].map(k=>`x4g-${k}-${x.run}`);
...
async function main(){
 ...
 try{
  ...
  // Reserve authority is reread after the internal writer's final item-row
  // wait, not merely after the outer advisory lock.
  ...
  const reserveLot=reserveAuth.lines[0].lot_id;
  await A.query('begin');await A.query('select id from public.e10_inventory_items where organization_id=$1 and id=$2 for update',[org,x.items[10]]);
  ...
  if(!reserveAuthError||reserveAuthError.code!=='42501'||reserveAuthError.message!=='reserve_inventory_denied')throw new Error(`reserve post-lock authority escaped ${reserveAuthError&&reserveAuthError.code}:${reserveAuthError&&reserveAuthError.message}`);
  ...
  // Lot consume: membership changes while the internal writer waits on its
  // final inventory-item row. The wrapper rolls back every transition.
  const consumeAuth=await receive(setup,x.items[12],`x4g-${x.run}-consumeauth-receive`);await setup.query('begin');await auth(setup);await dispose(setup,consumeAuth,'accept',2,null,0,'consume stock',`x4g-${x.run}-consumeauth-disposition`);const consumeReservation=(await setup.query('select public.e10_org_lot_reserve($1,$2,1,$3,$4) r',[org,consumeAuth.lines[0].lot_id,x.session,`x4g-${x.run}-consumeauth-reserve`])).rows[0].r;await setup.query('commit');
  await A.query('begin');await A.query('select id from public.e10_inventory_items where organization_id=$1 and id=$2 for update',[org,x.items[12]]);await B.query('begin');await auth(B);const consumeAuthPid=(await B.query('select pg_backend_pid() pid')).rows[0].pid;let consumeAuthError;const consumeAuthCall=B.query('select public.e10_org_lot_consume($1,$2,1,$3)',[org,consumeReservation.reservation_id,`x4g-${x.run}-consumeauth`]).catch(e=>{consumeAuthError=e;});await waitForLock(setup,consumeAuthPid,'consume membership');await setup.query("update public.e10_organization_memberships set status='suspended' where organization_id=$1 and user_id=$2",[org,x.user]);await A.query('commit');await consumeAuthCall;await B.query('rollback').catch(()=>{});if(!consumeAuthError||consumeAuthError.code!=='42501')throw new Error(`consume membership race escaped ${consumeAuthError&&consumeAuthError.message}`);const consumeResidue=(await setup.query('select status,consumed_quantity,(select count(*) from public.e10_lot_reservation_transitions where organization_id=$1 and lot_reservation_id=$2) transitions from public.e10_lot_reservations where organization_id=$1 and id=$2',[org,consumeReservation.reservation_id])).rows[0];if(consumeResidue.status!=='active'||Number(consumeResidue.consumed_quantity)!==0||Number(consumeResidue.transitions)!==0)throw new Error(`consume residue ${JSON.stringify(consumeResidue)}`);await setup.query("update public.e10_organization_memberships set status='active' where organization_id=$1 and user_id=$2",[org,x.user]);

  // Lot release: organization suspension at the same final row wait fails
  // closed and preserves the active reservation.
  const releaseAuth=await receive(setup,x.items[13],`x4g-${x.run}-releaseauth-receive`);await setup.query('begin');await auth(setup);await dispose(setup,releaseAuth,'accept',2,null,0,'release stock',`x4g-${x.run}-releaseauth-disposition`);const releaseReservation=(await setup.query('select public.e10_org_lot_reserve($1,$2,1,$3,$4) r',[org,releaseAuth.lines[0].lot_id,x.session,`x4g-${x.run}-releaseauth-reserve`])).rows[0].r;await setup.query('commit');
  await A.query('begin');await A.query('select id from public.e10_inventory_items where organization_id=$1 and id=$2 for update',[org,x.items[13]]);await B.query('begin');await auth(B);const releasePid=(await B.query('select pg_backend_pid() pid')).rows[0].pid;let releaseError;const releaseCall=B.query('select public.e10_org_lot_release($1,$2,$3)',[org,releaseReservation.reservation_id,`x4g-${x.run}-releaseauth`]).catch(e=>{releaseError=e;});await waitForLock(setup,releasePid,'release organization');await setup.query("update public.e10_organizations set status='suspended' where id=$1",[org]);await A.query('commit');await releaseCall;await B.query('rollback').catch(()=>{});if(!releaseError||releaseError.code!=='42501')throw new Error(`release organization race escaped ${releaseError&&releaseError.message}`);if((await setup.query('select status from public.e10_lot_reservations where organization_id=$1 and id=$2',[org,releaseReservation.reservation_id])).rows[0].status!=='active')throw new Error('release left reservation residue');await setup.query("update public.e10_organizations set status='active' where id=$1",[org]);

  // Batch reversal: destination authority disappears while the internal batch
  // waits on its last item row. All reversal and origin work rolls back.
  const batchLoc=await receive(setup,x.items[14],`x4g-${x.run}-batchloc-receive`);await setup.query('begin');await auth(setup);await dispose(setup,batchLoc,'accept',1,null,0,'batch location stock',`x4g-${x.run}-batchloc-disposition`);await setup.query('commit');await A.query('begin');await A.query('select id from public.e10_inventory_items where organization_id=$1 and id=$2 for update',[org,x.items[14]]);await B.query('begin');await auth(B);const batchPid=(await B.query('select pg_backend_pid() pid')).rows[0].pid;let batchError;const batchCall=B.query('select public.e10_org_reverse_receipt_batch($1,$2,$3,$4)',[org,batchLoc.receipt_id,'location revoked',`x4g-${x.run}-batchloc-reverse`]).catch(e=>{batchError=e;});await waitForLock(setup,batchPid,'batch reversal location');await setup.query('delete from public.e10_location_role_permissions where organization_id=$1 and location_id=$2 and role_id=$3',[org,x.location,x.role]);await A.query('commit');await batchCall;await B.query('rollback').catch(()=>{});if(!batchError||batchError.code!=='42501'||batchError.message!=='resolve_recovery_denied')throw new Error(`batch location race escaped ${batchError&&batchError.message}`);const batchResidue=(await setup.query("select sr.status,i.qty,(select count(*) from public.e10_stock_receipt_reversals where organization_id=$1 and stock_receipt_id=$2) reversals,(select count(*) from public.e10_receipt_reversal_commands where organization_id=$1 and stock_receipt_id=$2) commands from public.e10_stock_receipts sr join public.e10_inventory_items i on i.organization_id=sr.organization_id and i.id=$3 where sr.organization_id=$1 and sr.id=$2",[org,batchLoc.receipt_id,x.items[14]])).rows[0];if(batchResidue.status!=='posted'||Number(batchResidue.qty)!==1||Number(batchResidue.reversals)!==0||Number(batchResidue.commands)!==0)throw new Error(`batch reversal residue ${JSON.stringify(batchResidue)}`);await setup.query('insert into public.e10_location_role_permissions(organization_id,location_id,role_id,can_receive)values($1,$2,$3,true)',[org,x.location,x.role]);

  const singleAuth=await receive(setup,x.items[18],`x4g-${x.run}-singleauth-receive`);await setup.query('begin');await auth(setup);await dispose(setup,singleAuth,'accept',1,null,0,'single authority stock',`x4g-${x.run}-singleauth-disposition`);await setup.query('commit');await A.query('begin');await A.query('select id from public.e10_inventory_items where organization_id=$1 and id=$2 for update',[org,x.items[18]]);await B.query('begin');await auth(B);const singleAuthPid=(await B.query('select pg_backend_pid() pid')).rows[0].pid;let singleAuthError;const singleAuthCall=B.query('select public.e10_org_reverse_receipt($1,$2,$3,$4)',[org,singleAuth.receipt_id,'capability revoked',`x4g-${x.run}-singleauth-reverse`]).catch(e=>{singleAuthError=e;});await waitForLock(setup,singleAuthPid,'single reversal capability');await setup.query("delete from public.e10_organization_role_permissions where organization_id=$1 and role_id=$2 and capability='act.resolve_recovery'",[org,x.role]);await A.query('commit');await singleAuthCall;await B.query('rollback').catch(()=>{});if(!singleAuthError||singleAuthError.code!=='42501')throw new Error('single reversal authority escaped');if((await setup.query('select status from public.e10_stock_receipts where organization_id=$1 and id=$2',[org,singleAuth.receipt_id])).rows[0].status!=='posted')throw new Error('single reversal left receipt residue');await setup.query("insert into public.e10_organization_role_permissions(organization_id,role_id,capability,allowed)values($1,$2,'act.resolve_recovery',true)",[org,x.role]);

  // Historical missing-origin repair itself is serialized on the receipt. If
  // authority disappears during that wait, the attempted repair also rolls back.
  const originRace=await receive(setup,x.items[19],`x4g-${x.run}-originrace-receive`);await setup.query('set session_replication_role=replica');await setup.query("delete from public.e10_commercial_events where organization_id=$1 and event_type='receipt' and payload->>'receipt_line_id'=$2",[org,originRace.lines[0].receipt_line_id]);await setup.query('set session_replication_role=origin');await A.query('begin');await A.query('select id from public.e10_stock_receipts where organization_id=$1 and id=$2 for update',[org,originRace.receipt_id]);await B.query('begin');await auth(B);const originPid=(await B.query('select pg_backend_pid() pid')).rows[0].pid;let originError;const originCall=B.query('select public.e10_org_reverse_receipt_batch($1,$2,$3,$4)',[org,originRace.receipt_id,'origin repair race',`x4g-${x.run}-originrace-reverse`]).catch(e=>{originError=e;});await waitForLock(setup,originPid,'origin repair capability');await setup.query("delete from public.e10_organization_role_permissions where organization_id=$1 and role_id=$2 and capability='act.resolve_recovery'",[org,x.role]);await A.query('commit');await originCall;await B.query('rollback').catch(()=>{});if(!originError||originError.code!=='42501')throw new Error('origin repair authority escaped');const originResidue=(await setup.query("select (select count(*) from public.e10_commercial_events where organization_id=$1 and event_type='receipt' and payload->>'receipt_line_id'=$2) origins,(select count(*) from public.e10_receipt_reversal_commands where organization_id=$1 and stock_receipt_id=$3) commands",[org,originRace.lines[0].receipt_line_id,originRace.receipt_id])).rows[0];if(Number(originResidue.origins)!==0||Number(originResidue.commands)!==0)throw new Error(`origin repair residue ${JSON.stringify(originResidue)}`);await setup.query("insert into public.e10_organization_role_permissions(organization_id,role_id,capability,allowed)values($1,$2,'act.resolve_recovery',true)",[org,x.role]);

  // Legacy reserve, release and consume remain granted. Exercise each behind
  // its item-row lock with a different mutable authority dimension.
  await setup.query('update public.e10_inventory_items set qty=3 where organization_id=$1 and id=any($2::text[])',[org,x.items.slice(15,18)]);
  await A.query('begin');await A.query('select id from public.e10_inventory_items where organization_id=$1 and id=$2 for update',[org,x.items[15]]);await B.query('begin');await auth(B);const legacyReservePid=(await B.query('select pg_backend_pid() pid')).rows[0].pid;let legacyReserveError;const legacyReserveCall=B.query("select public.e10_org_inv_reserve($1,$2,'legacy-r','Legacy',1,$3)",[org,x.items[15],`x4g-${x.run}-legacy-reserve`]).catch(e=>{legacyReserveError=e;});await waitForLock(setup,legacyReservePid,'legacy reserve capability');await setup.query("delete from public.e10_organization_role_permissions where organization_id=$1 and role_id=$2 and capability='act.inventory_edit'",[org,x.role]);await A.query('commit');await legacyReserveCall;await B.query('rollback').catch(()=>{});if(!legacyReserveError||legacyReserveError.code!=='42501')throw new Error('legacy reserve authority escaped');if(Number((await setup.query('select count(*) c from public.e10_inventory_reservations where organization_id=$1 and item_id=$2',[org,x.items[15]])).rows[0].c)!==0)throw new Error('legacy reserve residue');await setup.query("insert into public.e10_organization_role_permissions(organization_id,role_id,capability,allowed)values($1,$2,'act.inventory_edit',true)",[org,x.role]);
  await setup.query('begin');await auth(setup);await setup.query("select public.e10_org_inv_reserve($1,$2,'legacy-rel','Legacy',1,$3)",[org,x.items[16],`x4g-${x.run}-legacy-release-seed`]);await setup.query('commit');await A.query('begin');await A.query('select id from public.e10_inventory_items where organization_id=$1 and id=$2 for update',[org,x.items[16]]);await B.query('begin');await auth(B);const legacyReleasePid=(await B.query('select pg_backend_pid() pid')).rows[0].pid;let legacyReleaseError;const legacyReleaseCall=B.query("select public.e10_org_inv_release($1,$2,'legacy-rel',$3)",[org,x.items[16],`x4g-${x.run}-legacy-release`]).catch(e=>{legacyReleaseError=e;});await waitForLock(setup,legacyReleasePid,'legacy release membership');await setup.query("update public.e10_organization_memberships set status='suspended' where organization_id=$1 and user_id=$2",[org,x.user]);await A.query('commit');await legacyReleaseCall;await B.query('rollback').catch(()=>{});if(!legacyReleaseError||legacyReleaseError.code!=='42501')throw new Error('legacy release authority escaped');if((await setup.query("select status from public.e10_inventory_reservations where organization_id=$1 and item_id=$2 and show_ref='legacy-rel'",[org,x.items[16]])).rows[0].status!=='active')throw new Error('legacy release residue');await setup.query("update public.e10_organization_memberships set status='active' where organization_id=$1 and user_id=$2",[org,x.user]);
  await A.query('begin');await A.query('select id from public.e10_inventory_items where organization_id=$1 and id=$2 for update',[org,x.items[17]]);await B.query('begin');await auth(B);const legacyConsumePid=(await B.query('select pg_backend_pid() pid')).rows[0].pid;let legacyConsumeError;const legacyConsumeCall=B.query('select public.e10_org_inv_consume($1,$2,null,null,1,$3)',[org,x.items[17],`x4g-${x.run}-legacy-consume`]).catch(e=>{legacyConsumeError=e;});await waitForLock(setup,legacyConsumePid,'legacy consume organization');await setup.query("update public.e10_organizations set status='suspended' where id=$1",[org]);await A.query('commit');await legacyConsumeCall;await B.query('rollback').catch(()=>{});if(!legacyConsumeError||legacyConsumeError.code!=='42501')throw new Error('legacy consume authority escaped');if(Number((await setup.query('select qty from public.e10_inventory_items where organization_id=$1 and id=$2',[org,x.items[17]])).rows[0].qty)!==3)throw new Error('legacy consume residue');await setup.query("update public.e10_organizations set status='active' where id=$1",[org]);
