...
declare o uuid:='e1000000-0000-4000-8000-0000000000a6'; other_org uuid:=gen_random_uuid(); u uuid:=gen_random_uuid(); role_id uuid:=gen_random_uuid();
  product uuid:=gen_random_uuid(); batch uuid; row1 uuid; row2 uuid; result jsonb; c bigint; occurred timestamptz; revision bigint;
begin
  ...
  result:=public.e10_org_stage_intake(o,'csv','x5d-source','x5d-doc','storage://x5d.csv','x5d-fp',jsonb_build_array(
    ...
    jsonb_build_object('raw_payload',jsonb_build_object('title','excluded'),'observation_kind','estimated_value','occurred_at','2025-01-03T03:04:05Z','currency','CAD','amount',30)),'x5d-stage');
  ...
  perform public.e10_org_resolve_intake_row(o,row2,'reject',null,'not reliable',null,'x5d-resolve-2');
  ...
  if (result->>'observation_count')::int<>1 or (result->>'rejected_row_count')::int<>1 or (result->>'replay')::boolean then raise exception 'commit result wrong %',result; end if;
  ...
  if c<>1 or occurred<>'2025-01-02T03:04:05Z' then raise exception 'observation evidence wrong count=% occurred=%',c,occurred; end if;
  result:=public.e10_org_commit_intake(o,batch,revision,'x5d-commit');
