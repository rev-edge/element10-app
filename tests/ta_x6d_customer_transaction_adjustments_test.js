...
async function main() {
  ...
  try {
    ...
    const newSource=`${run}-reviewed-new-source`;
    const newDraft=(await a.query("select public.e10_org_create_customer_transaction_draft($1,$2,'CAD','2026-01-02T00:00:00Z','exact',$3,$4,$5) r",[org,ids.customer,`X6d reviewed new ${run}`,JSON.stringify([{purchase_kind:'retail',capture_source:'import',source_connection_id:'reviewed-new-file',source_line_id:newSource,quantity:1,merchandise_gross:11,merchandise_discount:0}]),`${run}-reviewed-new-draft`])).rows[0].r.draft_id;draftIds.push(newDraft);
    await a.query('select public.e10_org_approve_customer_transaction_draft($1,$2,1,$3)',[org,newDraft,`${run}-reviewed-new-approve`]);
    const newCase=(await a.query(openRecon,[org,'import','reviewed-new-file',`${run}-reviewed-new-event`,newSource,'CAD',11,null,null,JSON.stringify({review_required:true}),`${run}-reviewed-new-open`])).rows[0].r;
    await a.query(decideRecon,[org,newCase.case_id,0,'new_transaction',null,null,null,'review confirms distinct new sale',JSON.stringify({reviewed:true}),`${run}-reviewed-new-decision`]);
    ...
    const eventLocalClaims=(await admin.query("select count(*)::int n from public.e10_customer_transaction_source_claims where organization_id=$1 and source_kind='import' and source_connection_id='event-local-file' and source_line_id=any($2::text[])",[org,[eventLocalA,eventLocalB]])).rows[0].n;
    if(eventLocalClaims!==2)throw new Error('collision-safe event-local source identity contract failed');
