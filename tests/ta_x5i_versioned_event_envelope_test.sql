...
begin
  ...
  begin perform public.e10_org_record_commercial_event_v2(o,'listing_created',1,'unique_item',copy_id::text,now(),'exact','manual',null,null,'null-key','corr',null,'operator_asserted','{"listing_id":null,"channel":"x"}',null,'{}','x5i-null'); raise exception 'null required value accepted'; exception when sqlstate '22023' then null; end;
  begin perform public.e10_org_record_commercial_event_v2(o,'listing_created',1,'unique_item',copy_id::text,now(),'exact','manual',null,null,'wrong-type','corr',null,'operator_asserted','{"listing_id":"x","channel":[]}',null,'{}','x5i-type'); raise exception 'wrong payload type accepted'; exception when sqlstate '22023' then null; end;
  begin perform public.e10_org_record_commercial_event_v2(o,'fee',1,'unique_item',copy_id::text,now(),'exact','manual',null,null,'bad-amount','corr',null,'operator_asserted','{"fee_id":"x","currency":"CAD","amount":"NaN"}',null,'{}','x5i-amount-type'); raise exception 'string amount accepted'; exception when sqlstate '22023' then null; end;
  begin perform public.e10_org_record_commercial_event_v2(o,'fee',1,'unique_item',copy_id::text,now(),'exact','manual',null,null,'negative','corr',null,'operator_asserted','{"fee_id":"x","currency":"CAD","amount":-1}',null,'{}','x5i-negative'); raise exception 'negative amount accepted'; exception when sqlstate '22023' then null; end;
  begin perform public.e10_org_record_commercial_event_v2(o,'fee',1,'unique_item',copy_id::text,now(),'exact','manual',null,null,'bad-currency','corr',null,'operator_asserted','{"fee_id":"x","currency":"cad","amount":1}',null,'{}','x5i-currency'); raise exception 'invalid currency accepted'; exception when sqlstate '22023' then null; end;
  begin perform public.e10_org_record_commercial_event_v2(o,'return',1,'unique_item',copy_id::text,'infinity','exact','manual',null,null,'infinite-time','corr',null,'operator_asserted',payloads->'return',null,'{}','x5i-infinite'); raise exception 'infinite timestamp accepted'; exception when sqlstate '22023' then null; end;
  ...
  begin perform public.e10_org_record_commercial_event_v2(o,'return',1,'unique_item',copy_id::text,now(),'exact','native',null,null,'spoof-native','corr',null,'native_system',payloads->'return',null,'{}','x5i-native-v2'); raise exception 'v2 native provenance spoof accepted'; exception when insufficient_privilege then null; end;
  begin perform public.e10_org_record_commercial_event_v2(o,'return',1,'unique_item',copy_id::text,now(),'exact','manual',null,null,'spoof-quality','corr',null,'native_system',payloads->'return',null,'{}','x5i-quality'); raise exception 'native quality spoof accepted'; exception when insufficient_privilege then null; end;
  begin perform public.e10_org_record_commercial_event(o,'listing_created','inventory_item',item_id,now(),'native','spoof',jsonb_build_object('listing_id','spoof','channel','spoof'),null,'{}','x5i-native-v1'); raise exception 'v1 native provenance spoof accepted'; exception when insufficient_privilege then null; end;
  begin perform public.e10_org_record_commercial_event(o,'listing_created','inventory_item',item_id,now(),'system','spoof',jsonb_build_object('listing_id','spoof','channel','spoof'),null,'{}','x5i-system-v1'); raise exception 'v1 system provenance spoof accepted'; exception when insufficient_privilege then null; end;
  begin perform public.e10_org_record_commercial_event(o,'listing_created','inventory_item',item_id,now(),'manual','missing','{}',null,'{}','x5i-v1-missing'); raise exception 'v1 missing keys accepted'; exception when sqlstate '22023' then null; end;
  begin perform public.e10_org_record_commercial_event(o,'fee','inventory_item',item_id,now(),'manual','missing-amount','{"fee_id":"x","currency":"CAD"}',null,'{}','x5i-v1-amount'); raise exception 'v1 missing amount accepted'; exception when sqlstate '22023' then null; end;
  begin perform public.e10_org_record_commercial_event(o,'return','inventory_item',item_id,'infinity','manual','infinite','{"return_id":"x"}',null,'{}','x5i-v1-infinite'); raise exception 'v1 infinite timestamp accepted'; exception when check_violation then null; end;
