...
declare
  ...
  invoice uuid:=gen_random_uuid(); invoice_line uuid:=gen_random_uuid();
  credit uuid:=gen_random_uuid(); credit_line uuid:=gen_random_uuid();
  invoice_event uuid:=gen_random_uuid(); credit_event uuid:=gen_random_uuid();
begin
  ...
  if (select count(*) from public.e10_financial_allocation_commands where organization_id=o)<>2
    or (select count(*) from public.e10_invoice_po_allocation_events where organization_id=o)<>1
    or (select count(*) from public.e10_credit_invoice_allocation_events where organization_id=o)<>1 then
