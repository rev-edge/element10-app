...
begin
 ...
 j:=public.e10_org_inventory_lifecycle(o,'2026-01-06',item_late,10,null);if j#>>'{items,0,disposition_kind}'is distinct from'disposal'then raise exception'initial receipt disposal missing %',j;end if;
 reset role;
 ...
 j:=public.e10_org_inventory_lifecycle(o,'2026-01-06',item_late,10,null);if j#>>'{items,0,origin_event_id}'is distinct from acquisition_id::text or j#>>'{items,0,disposition_kind}'is distinct from'disposal'or(j#>>'{items,0,censored}')::boolean is distinct from false then raise exception'late acquisition/correction detached finality %',j;end if;
 ...
 j:=public.e10_org_inventory_lifecycle(o,'2026-01-06',item_late,10,null);if(j#>>'{items,0,censored}')::boolean is distinct from true then raise exception'corrected episode could not be revoked %',j;end if;
 ...
 set local role authenticated;
 j:=public.e10_org_inventory_lifecycle(o,'2026-01-06',item_unresolved,10,null);if j#>>'{items,0,availability}'is distinct from'finality_conflict'or(j#>>'{items,0,finality_conflict}')::boolean is distinct from true then raise exception'unresolved corrected episode was reported as ordinary holding %',j;end if;
 j:=public.e10_org_inventory_valuation_coverage(o,'manual','1','USD','2026-01-06',null,365,10,null);j2:=jsonb_path_query_first(j,'$.items[*] ? (@.unique_item_id == $id)',jsonb_build_object('id',item_unresolved::text));if j2 is null or j2->>'holding_evidence'is distinct from'episode_finality_conflict'or j2->'value'is distinct from'null'::jsonb then raise exception'unresolved corrected episode contributed normal value %',j;end if;

 j:=public.e10_org_inventory_lifecycle(o,'2026-01-10',item_cutoff,10,null);if j#>>'{items,0,disposition_source_basis}'is distinct from'trusted_posted_transaction'then raise exception'future reviewed finality erased early native finality %',j;end if;
 ...
 j:=public.e10_org_inventory_lifecycle(o,'2026-01-10',item_cutoff,10,null);if j#>>'{items,0,disposition_source_basis}'is distinct from'trusted_posted_transaction'then raise exception'future reviewed finality changed early cutoff %',j;end if;
 j:=public.e10_org_inventory_lifecycle(o,'2026-01-21',item_cutoff,10,null);if j#>>'{items,0,disposition_source_basis}'is distinct from'reviewed_link'or(j#>>'{items,0,finality_conflict}')::boolean is distinct from false then raise exception'reviewed finality did not take cutoff-local precedence %',j;end if;
 ...
 j:=public.e10_org_inventory_lifecycle(o,'2026-01-03',item_overlap,10,null);j2:=jsonb_path_query_first(j,'$.items[*] ? (@.origin_event_id == $id)',jsonb_build_object('id',overlap_a::text));if j2 is null or j2->>'availability'is distinct from'episode_ambiguous'or(j2->>'overlapping_origin')::boolean is distinct from true then raise exception'unclosed prior episode labeled available %',j;end if;
 reset role;
end $$;
rollback;
