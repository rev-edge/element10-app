...
begin
 ...
 bs:=pg_temp.x8_business_state();j:=public.e10_org_typed_query(o,x8ctx,'supplier.workspace',jsonb_build_object('supplier_id',supplier,'limit',100));if pg_temp.x8_business_state()is distinct from bs then raise exception'X8 supplier workspace changed business data';end if;perform pg_temp.x8_assert_envelope(j,'supplier.workspace','supplier_document');if jsonb_array_length(j#>'{result,items}')<>5 or j->'cutoff'='null'::jsonb or not exists(select 1 from jsonb_array_elements(j->'sources')as src(value)where src.value->>'kind'='supplier_invoice'and(src.value->>'revision')::integer=1 and src.value->>'document_id'=invoice_id::text)then raise exception'X8 supplier workspace semantic envelope %',j;end if;
