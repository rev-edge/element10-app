...
async function main() {
  ...
  try {
    ...
    await c.query("insert into public.e10_session_presence_streams(id,organization_id,session_id,source_class,provider_key,subject_key,connection_id,platform_attendee_key,original_customer_id,identity_status,collection_policy_version,notice_version,coverage_label,retention_expires_at)values($1,$2,$3,'authorized_platform','provider-a','attendee','connection','attendee',$4,'reviewed_attributed',1,'n1','fixture','2027-01-01')",[x.stream,x.org,x.session,x.customer]);
    ...
    await c.query("insert into public.e10_session_presence_streams(id,organization_id,session_id,source_class,provider_key,subject_key,connection_id,platform_attendee_key,original_customer_id,identity_status,collection_policy_version,notice_version,coverage_label,retention_expires_at)values($1,$2,$3,'authorized_platform','provider-a','attendee-2','connection-2','attendee-2',$4,'reviewed_attributed',1,'n1','fixture','2027-01-01')",[x.stream2,x.org,x.session,x.customer]);
    ...
    const cutoff2=(await c.query("select public.e10_service_run_provider_presence_normalization($1,'provider-a',$2,'2026-01-01','2026-02-01','2026-02-02',100,'run-cutoff-2') result",[x.org,x.policy])).rows[0].result;
    ...
    const coexist=Number((await c.query("select ((e10.current_provider_normalization_run($1,'provider-a','2026-01-01','2026-02-01','2026-02-01')).id is not null)::int+((e10.current_provider_normalization_run($1,'provider-a','2026-01-01','2026-02-01','2026-02-02')).id is not null)::int+((e10.current_provider_normalization_run($1,'provider-b','2026-01-01','2026-02-01','2026-02-01')).id is not null)::int n",[x.org])).rows[0].n);
    ...
    if(!await rejected(c.query("select * from public.e10_org_source_attendance_summary($1,'provider-a',$2,$3,'2026-01-01','2026-02-01','2026-02-01')",[x.org,randomUUID(),x.customer]),"22023"))throw Error("foreign summary session accepted");
    const qa=(await c.query("select * from public.e10_org_provider_attendance_quarantine($1,'provider-a','2026-01-01','2026-02-01','2026-02-01','missing_provider_time',2,null,null,null)",[x.org])).rows[0];
