...
begin;
do $$
declare o uuid:=gen_random_uuid();release_id uuid:=gen_random_uuid();variant_id uuid:=gen_random_uuid();copy3 uuid:=gen_random_uuid();copy4 uuid:=gen_random_uuid();before_inventory bigint;before_customer bigint;
begin
 ...
 if(select count(*)from public.e10_inventory_movements)<>before_inventory or(select count(*)from public.e10_customer_transactions)<>before_customer then raise exception'market evidence changed inventory or customer truth';end if;
end $$;
rollback;
