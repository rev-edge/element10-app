...
const x = { org:id(1), actor:id(2), role:id(3), supplier:id(4), location:id(5), product:id(6), config:id(7), version:id(8),
  po:id(9), pol:id(10), inv1:id(11), il1:id(12), inv2:id(13), il2:id(14), inv3:id(15), il3:id(16), po2:id(17), pol2:id(18),
  credit1:id(19), cl1:id(20), credit2:id(21), cl2:id(22) };
...
async function setup(){
 ...
 await admin.query("insert into public.e10_purchase_orders(id,organization_id,supplier_id,destination_location_id,order_number,status,currency,created_by) values($1,$2,$3,$4,$5,'approved','CAD',$6)",[x.po2,x.org,x.supplier,x.location,`PO2-${run}`,x.actor]);
 ...
 await admin.query('insert into public.e10_purchase_order_lines(id,organization_id,purchase_order_id,configuration_version_id,line_no,ordered_quantity) values($1,$2,$3,$4,1,10)',[x.pol2,x.org,x.po2,x.version]);
 ...
 for(const [credit,line,no] of [[x.credit1,x.cl1,1],[x.credit2,x.cl2,2]]){await admin.query("insert into public.e10_supplier_credits(id,organization_id,supplier_id,supplier_document_number,currency,total_amount,created_by) values($1,$2,$3,$4,'CAD',10,$5)",[credit,x.org,x.supplier,`CR-${run}-${no}`,x.actor]);await admin.query('insert into public.e10_supplier_credit_lines(id,organization_id,supplier_credit_id,configuration_version_id,line_no,line_amount) values($1,$2,$3,$4,1,10)',[line,x.org,credit,x.version]);}
}
async function main(){await Promise.all([admin.connect(),a.connect(),b.connect()]);await setup();
 ...
 await admin.query("set constraints all deferred; set session_replication_role='replica'");
 await admin.query('delete from public.e10_invoice_po_allocation_events where organization_id=$1',[x.org]);
 await admin.query('delete from public.e10_financial_allocation_commands where organization_id=$1',[x.org]);
 await admin.query('delete from public.e10_supplier_invoice_revisions where organization_id=$1',[x.org]);
 await admin.query('delete from public.e10_invoice_po_allocations where organization_id=$1',[x.org]);
 await admin.query("update public.e10_supplier_invoices set revision=1,status='draft',reviewed_by=null,reviewed_at=null,approved_revision=null,approved_by=null,approved_at=null where organization_id=$1",[x.org]);
 await admin.query("set session_replication_role='origin'");
 await admin.query('insert into public.e10_invoice_po_allocations(organization_id,invoice_line_id,purchase_order_line_id,allocated_quantity) values($1,$2,$3,1),($1,$4,$5,1)',[x.org,x.il1,x.pol2,x.il2,x.pol]);
 await a.query('begin');await b.query('begin');await claims(a);await claims(b);
 const crossA=a.query('select public.e10_org_allocate_invoice_to_po($1,$2,$3,1,1,1,$4,$4) r',[x.org,x.il1,x.pol,`cross-a-${run}`]);const crossB=b.query('select public.e10_org_allocate_invoice_to_po($1,$2,$3,1,1,1,$4,$4) r',[x.org,x.il2,x.pol2,`cross-b-${run}`]);
 const cross=await bounded(Promise.all([crossA,crossB]));await a.query('commit');await b.query('commit');
 if(cross.some(q=>q.rows[0].r.supplier_invoice_revision!==2))throw Error('crossmatched graph result invalid');
 console.log('[proof] crossmatched invoice/PO allocation graph completed without deadlock under bounded wait');
 ...
 await admin.query("update public.e10_supplier_invoices set revision=1,status='draft',reviewed_by=null,reviewed_at=null,approved_revision=null,approved_by=null,approved_at=null where organization_id=$1",[x.org]);
 await admin.query("set session_replication_role='origin'");
 await admin.query('insert into public.e10_credit_invoice_allocations(organization_id,credit_line_id,invoice_line_id,allocated_amount) values($1,$2,$3,1),($1,$4,$5,1)',[x.org,x.cl1,x.il2,x.cl2,x.il1]);
 await a.query('begin');await b.query('begin');await claims(a);await claims(b);
 const creditA=a.query('select public.e10_org_allocate_credit_to_invoice($1,$2,$3,1,1,1,$4,$4) r',[x.org,x.cl1,x.il1,`credit-cross-a-${run}`]);
 const creditB=b.query('select public.e10_org_allocate_credit_to_invoice($1,$2,$3,1,1,1,$4,$4) r',[x.org,x.cl2,x.il2,`credit-cross-b-${run}`]);
 const creditCross=await bounded(Promise.all([creditA,creditB]));await a.query('commit');await b.query('commit');
 if(creditCross.some(q=>q.rows[0].r.supplier_credit_revision!==2||q.rows[0].r.supplier_invoice_revision!==2))throw Error('credit crossmatched graph result invalid');
 console.log('[proof] crossmatched credit/invoice allocation graph completed without deadlock under bounded wait');
