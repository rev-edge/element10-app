...
begin
 ...
 x8ctx:=(public.e10_org_create_query_context(o,'workspace',600,'x8-x7e1')->>'context_id')::uuid;j:=public.e10_org_typed_query(o,x8ctx,'inventory.lifecycle',jsonb_build_object('as_of','2026-01-10T00:00:00Z','unique_item_id',item1,'limit',100));if j->>'grain'<>'unique_item_ownership_episode'or j#>>'{result,items,0,unique_item_id}'<>item1::text then raise exception'X8 lifecycle dispatch %',j;end if;
