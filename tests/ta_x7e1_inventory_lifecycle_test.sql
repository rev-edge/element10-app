...
begin;
do $$
declare o uuid:=gen_random_uuid();user_id uuid:=gen_random_uuid();role_id uuid:=gen_random_uuid();release_id uuid:=gen_random_uuid();variant_id uuid:=gen_random_uuid();item1 uuid:=gen_random_uuid();item2 uuid:=gen_random_uuid();item3 uuid:=gen_random_uuid();item4 uuid:=gen_random_uuid();item5 uuid:=gen_random_uuid();origin1 uuid:=gen_random_uuid();origin2 uuid:=gen_random_uuid();origin3 uuid:=gen_random_uuid();origin4 uuid:=gen_random_uuid();receipt4 uuid:=gen_random_uuid();origin5 uuid:=gen_random_uuid();tx_id uuid:=gen_random_uuid();tx2 uuid:=gen_random_uuid();tx3 uuid:=gen_random_uuid();sale_obs uuid:=gen_random_uuid();replacement_obs uuid:=gen_random_uuid();external_obs uuid:=gen_random_uuid();j jsonb;j2 jsonb;cursor_value text;
begin
 ...
 (tx3,o,'USD','2026-01-09 00:00Z','exact',gen_random_uuid(),1,gen_random_uuid(),user_id,'2026-01-09 00:01Z');
 ...
 j:=public.e10_org_inventory_lifecycle(o,'2026-01-10 00:00Z',item5,100,null);if(j#>>'{items,0,censored}')::boolean is not true then raise exception'provisional or conflicting native sales closed ownership %',j;end if;
