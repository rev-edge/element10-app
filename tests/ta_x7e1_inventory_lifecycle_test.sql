...
begin;
do $$
declare o uuid:=gen_random_uuid();user_id uuid:=gen_random_uuid();role_id uuid:=gen_random_uuid();release_id uuid:=gen_random_uuid();variant_id uuid:=gen_random_uuid();item1 uuid:=gen_random_uuid();item2 uuid:=gen_random_uuid();item3 uuid:=gen_random_uuid();item4 uuid:=gen_random_uuid();item5 uuid:=gen_random_uuid();origin1 uuid:=gen_random_uuid();origin2 uuid:=gen_random_uuid();origin3 uuid:=gen_random_uuid();origin4 uuid:=gen_random_uuid();origin5 uuid:=gen_random_uuid();tx_id uuid:=gen_random_uuid();sale_obs uuid:=gen_random_uuid();replacement_obs uuid:=gen_random_uuid();external_obs uuid:=gen_random_uuid();j jsonb;j2 jsonb;cursor_value text;
begin
 ...
 insert into public.e10_unique_items(id,organization_id,catalog_variant_id,item_kind)values(item1,o,variant_id,'card'),(item2,o,variant_id,'card'),(item3,o,variant_id,'card'),(item4,o,variant_id,'card'),(item5,o,variant_id,'card');
 ...
 (gen_random_uuid(),o,'receipt',1,'unique_item',item4::text,'2026-01-02 00:00Z','exact','x7el-receipt4','manual','operator_asserted',jsonb_build_object('receipt_id','r4'),user_id),
 ...
 insert into public.e10_customer_transactions(id,organization_id,currency,occurred_at,occurred_at_precision,source_draft_id,source_draft_revision,commercial_event_id,posted_by,posted_at)values(tx_id,o,'USD','2026-01-09 00:00Z','exact',gen_random_uuid(),1,gen_random_uuid(),user_id,'2026-01-09 00:01Z');
 insert into public.e10_customer_transaction_lines(organization_id,transaction_id,line_no,purchase_kind,capture_source,source_line_id,unique_item_id,quantity,merchandise_gross,merchandise_discount,shipping_amount,tax_amount,raw_evidence)values(o,tx_id,1,'retail','native','x7el-native-line',item4,1,200,0,0,0,'{}');
 ...
 j:=public.e10_org_inventory_lifecycle(o,'2026-01-10 00:00Z',item4,100,null);if jsonb_array_length(j->'items')<>1 or j#>>'{items,0,origin_event_id}'<>origin4::text or j#>>'{items,0,disposition_source_basis}'<>'trusted_posted_transaction'or(j#>>'{items,0,censored}')::boolean then raise exception'native sale or correlated receipt episode invalid %',j;end if;
 j:=public.e10_org_inventory_lifecycle(o,'2026-01-10 00:00Z',item5,100,null);if(j#>>'{items,0,censored}')::boolean is not true then raise exception'provisional sale closed ownership %',j;end if;
 ...
 j:=public.e10_org_inventory_lifecycle(o,'2026-01-10 00:00Z',null,1,null);cursor_value:=j->>'next_cursor';if(j->>'total_count')::int<>5 or(j#>>'{exclusions,missing_occurrence_count}')::int<>1 or(j#>>'{exclusions,ambiguous_identity_count}')::int<>1 or jsonb_array_length(j->'items')<>1 or cursor_value is null then raise exception'bounded page one invalid %',j;end if;
 j2:=public.e10_org_inventory_lifecycle(o,'2026-01-10 00:00Z',null,1,cursor_value);if(j2->>'total_count')::int<>5 or jsonb_array_length(j2->'items')<>1 then raise exception'bounded page two invalid %',j2;end if;
