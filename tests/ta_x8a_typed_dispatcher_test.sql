...
begin
 ...
 begin perform public.e10_org_typed_query(o,cid,'inventory.history',jsonb_build_object('limit',10,'after_created','infinity','after_id',gen_random_uuid()));raise exception'positive infinity cursor accepted';exception when invalid_parameter_value then null;end;
 begin perform public.e10_org_typed_query(o,cid,'inventory.history',jsonb_build_object('limit',10,'after_created','-infinity','after_id',gen_random_uuid()));raise exception'negative infinity cursor accepted';exception when invalid_parameter_value then null;end;
 begin perform public.e10_org_typed_query(o,cid,'inventory.history','{"limit":10,"after_created":"2026-01-01T00:00:00Z","after_id":"not-a-uuid"}');raise exception'invalid UUID accepted';exception when invalid_parameter_value then null;end;
 begin perform public.e10_org_typed_query(o,cid,'inventory.page','{"limit":99999999999999999999,"filters":{}}');raise exception'overflowing integer accepted';exception when invalid_parameter_value then null;end;
