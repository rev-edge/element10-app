...
const x=Object.fromEntries(['user','role','supplier1','supplier2','location1','location2','product','config'].map(k=>[k,randomUUID()]));
...
async function main(){
  ...
  try{
    ...
    await A.query('begin');await A.query('select pg_advisory_xact_lock(hashtextextended($1,0))',[`${org}|receipt-command|${sameKey}`]);await auth(A);
    ...
    console.log(`TA-X4e locking races: PASS (same-key B pid=${bpid} replayed; authority B pid=${authPid} denied; opposite item order completed)`);
  }finally{
