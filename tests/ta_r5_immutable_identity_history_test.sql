...
declare
  ...
  release_id uuid:=gen_random_uuid();mapping1 uuid:=gen_random_uuid();mapping2 uuid:=gen_random_uuid();mapping3 uuid:=gen_random_uuid();
begin
  ...
  insert into public.e10_catalog_identity_mappings(id,provider,entity_kind,external_id,mapping_revision,release_id,match_status,is_current)
    values(mapping3,'r5-provider','release','r5-external',3,release_id,'candidate',false);
  begin update public.e10_catalog_identity_mappings set is_current=false where id=mapping1;
    raise exception'direct retirement with successors accepted';exception when sqlstate'55000'then null;end;
  begin update public.e10_catalog_identity_mappings set is_current=true where id=mapping2;
    raise exception'direct historical promotion accepted';exception when sqlstate'55000'then null;end;
  if (select count(*)from public.e10_catalog_identity_mappings where provider='r5-provider'and external_id='r5-external'and is_current)<>1
    or not exists(select 1 from public.e10_catalog_identity_mappings where id=mapping1 and is_current) then raise exception'interrupted/direct promotion changed current mapping';end if;
end $$;
rollback;
