...
async function main() {
  ...
  const fp = () =>
    c.query(
      "select e10.provider_normalization_dependency_fingerprint($1,'provider-a','2026-01-01T00:00:00Z','2026-02-01T00:00:00Z','2026-02-01T00:00:00Z',100) value",
      [x.org],
    );
  let ok = false;
  try {
    ...
    await c.query(
      "insert into public.e10_presence_collection_policies(organization_id,policy_version,source_class,provider_key,enabled,notice_version,heartbeat_expiry_seconds,min_event_interval_ms,max_events_per_minute,retention_interval,coverage_label,effective_from)values($1,1,'authorized_platform','provider-a',true,'n1',300,0,60,interval '30 days','fixture','2026-01-01'),($1,1,'authorized_platform','provider-b',true,'n1',300,0,60,interval '30 days','fixture','2026-01-01')",
      [x.org],
    );
    await c.query(
      "insert into public.e10_provider_presence_normalization_policy_decisions(id,organization_id,provider_key,revision,action,effective_from,effective_through,heartbeat_expiry,maximum_late_arrival,collection_policy_version,reason,evidence,idempotency_key,request_fingerprint)values($1,$2,'provider-a',1,'enable','2026-01-01','2027-01-01',interval '5 minutes',interval '1 day',1,'fixture','{}','policy','policy-fp')",
      [x.policy, x.org],
    );
    await c.query(
      "insert into public.e10_session_presence_streams(id,organization_id,session_id,source_class,provider_key,subject_key,connection_id,platform_attendee_key,identity_status,collection_policy_version,notice_version,coverage_label,retention_expires_at)values($1,$4,$5,'authorized_platform','provider-a','attendee-a','connection-a','attendee-a','unresolved',1,'n1','fixture','2027-01-01'),($2,$4,$5,'authorized_platform','provider-b','attendee-b','connection-b','attendee-b','unresolved',1,'n1','fixture','2027-01-01'),($3,$4,$6,'authorized_platform','provider-a','unrelated','unrelated','unrelated','unresolved',1,'n1','fixture','2027-01-01')",
    ...
    await c.query(
      "insert into public.e10_provider_presence_normalization_policy_decisions(id,organization_id,provider_key,revision,action,effective_from,effective_through,heartbeat_expiry,maximum_late_arrival,collection_policy_version,reason,evidence,supersedes_decision_id,idempotency_key,request_fingerprint)values($1,$2,'provider-a',2,'revoke','2026-01-10','2026-01-11',null,null,1,'short suppressor','{}',$3,'revoke','revoke-fp')",
    ...
    await c.query(
      "insert into public.e10_provider_presence_normalization_policy_decisions(id,organization_id,provider_key,revision,action,effective_from,effective_through,heartbeat_expiry,maximum_late_arrival,collection_policy_version,reason,evidence,supersedes_decision_id,idempotency_key,request_fingerprint)values($1,$2,'provider-a',3,'supersede','2026-01-12','2027-01-01',interval '5 minutes',interval '1 day',1,'restore','{}',$3,'restore','restore-fp')",
    ...
    await c.query(
      "insert into public.e10_session_presence_events(id,organization_id,stream_id,segment_id,event_sequence,event_kind,server_received_at,provider_occurred_at,provider_event_id,provider_key,evidence,request_fingerprint)values($1,$2,$3,$4,1,'join','2026-01-02',null,'missing-time','provider-a','{}','event-missing')",
    ...
    await c.query(
      "insert into public.e10_session_presence_events(id,organization_id,stream_id,segment_id,event_sequence,event_kind,server_received_at,provider_occurred_at,provider_event_id,provider_key,evidence,request_fingerprint)values($1,$2,$3,$4,2,'heartbeat','2026-01-03','2027-01-03','future-time','provider-a','{}','event-future')",
    ...
    await c.query(
      "insert into public.e10_session_presence_events(id,organization_id,stream_id,segment_id,event_sequence,event_kind,server_received_at,provider_occurred_at,provider_event_id,provider_key,evidence,request_fingerprint)values($1,$2,$3,$4,1,'join','2025-01-02','2025-01-02','other-window','provider-a','{}','event-other-window')",
    ...
    await c.query(
      "insert into public.e10_session_presence_events(id,organization_id,stream_id,segment_id,event_sequence,event_kind,server_received_at,provider_occurred_at,provider_event_id,provider_key,evidence,request_fingerprint)values($1,$2,$3,$4,3,'leave','2026-02-02','2026-01-05','after-cutoff','provider-a','{}','event-late-receipt')",
    ...
    await c.query(
      "insert into public.e10_session_presence_events(id,organization_id,stream_id,segment_id,event_sequence,event_kind,server_received_at,provider_occurred_at,provider_event_id,provider_key,corrects_event_id,evidence,request_fingerprint)values($1,$2,$3,$4,4,'join','2026-03-01','2026-01-02','correction','provider-a',$5,'{}','event-correction')",
    ...
    const alternateCutoff = (
      await c.query(
        "select e10.provider_normalization_dependency_fingerprint($1,'provider-a','2026-01-01T00:00:00Z','2026-02-01T00:00:00Z','2026-02-02T00:00:00Z',100) value",
        [x.org],
      )
    ).rows[0].value;
    ...
    if (
      !(await rejected(
        c.query(
          "select e10.provider_normalization_dependency_fingerprint($1,'provider-a','2026-01-01T00:00:00Z','2026-02-01T00:00:00Z','2026-02-01T00:00:00Z',1)",
          [x.org],
        ),
        "54000",
    ...
    if (
      !(await rejected(
        c.query(
          "select e10.provider_normalization_dependency_fingerprint($1,'provider-a','2026-02-01T00:00:00Z','2026-01-01T00:00:00Z','2026-02-01T00:00:00Z',100)",
          [x.org],
        ),
        "22023",
    ...
    const aclDenied = await rejected(
      c.query(
        "select e10.provider_normalization_dependency_fingerprint($1,'provider-a','2026-01-01T00:00:00Z','2026-02-01T00:00:00Z','2026-02-01T00:00:00Z',100)",
        [x.org],
      ),
      "42501",
    );
    ...
    await c.query(
      "insert into public.e10_presence_collection_policies(organization_id,policy_version,source_class,provider_key,enabled,notice_version,heartbeat_expiry_seconds,min_event_interval_ms,max_events_per_minute,retention_interval,coverage_label,effective_from)values($1,1,'authorized_platform','provider-a',true,'n1',300,0,60,interval '30 days','foreign','2026-01-01')",
      [x.org2],
    );
