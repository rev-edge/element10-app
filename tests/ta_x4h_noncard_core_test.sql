...
declare
  ...
  config_id uuid:=gen_random_uuid(); invoice_id uuid:=gen_random_uuid(); invoice_line_id uuid:=gen_random_uuid();
  result jsonb; replay jsonb; history jsonb; lot_id uuid; reservation_id uuid; n bigint;
begin
  ...
  values(o,role_id,'act.create_receiving',true),(o,role_id,'act.purchasing_prepare',true),
    (o,role_id,'act.reserve_inventory',true),(o,role_id,'financial.actual_cost.read',true);
  ...
  values(product_id,o,'X4h apparel','active');
  ...
  values('x4h-apparel','X4h apparel cartons',0,o),('x4h-camera','X4h used camera',1,o);
