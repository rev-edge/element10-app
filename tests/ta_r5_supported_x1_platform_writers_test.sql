...
declare admin_user uuid:=gen_random_uuid();ordinary uuid:=gen_random_uuid();player uuid:=gen_random_uuid();
  release_result jsonb;variant_result jsonb;map1 jsonb;map2 jsonb;release_id uuid;variant_id uuid;
begin
  ...
  if (map1->>'mapping_revision')::integer<>1 or(map2->>'mapping_revision')::integer<>2
    ...
    or not exists(select 1 from public.e10_catalog_identity_mappings where id=(map2->>'mapping_id')::uuid and is_current) then raise exception'mapping revision chain invalid';end if;
