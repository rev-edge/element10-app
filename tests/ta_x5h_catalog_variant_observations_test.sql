...
declare
  ...
  release_id uuid:=gen_random_uuid(); variant_id uuid:=gen_random_uuid(); batch_a uuid; batch_b uuid; row_a uuid; row_b uuid; result jsonb;
begin
  ...
  if (select count(*) from public.e10_market_observations where catalog_variant_id=variant_id)<>2 then raise exception 'shared variant observations missing'; end if;
  ...
  raise notice 'TA-X5h catalog-variant observations: PASS (shared catalog target, tenant evidence, no ownership)';
end $$;
rollback;
